(function(){
  ...
  try{
    chrome.storage.local.get({mdlaug_store:null, mdlaug_turso:{url:"",token:"",assessor:""}}, function(s){
      var c = s.mdlaug_store || (function(){ var lg=s.mdlaug_turso||{}; return { storage:(lg.url&&lg.token)?"turso-cloud":"local", tursoUrl:lg.url||"", tursoToken:lg.token||"", assessor:lg.assessor||"" }; })();
...
(function(){
  ...
  var st=$("cfg-status");
  try{
    chrome.storage.sync.get({describeUrl:"",describeHeaders:"",ocrEnabled:false,concurrency:2,
      budget:{describeImage:"ask",describeGraph:"ask",ocr:"ask",reflowPdf:"auto"}}, function(cfg){
  ...
  $("cfg-save").addEventListener("click", function(){
    ...
    var cfg={ describeUrl:$("cfg-url").value.trim(), describeHeaders:headers,
      ...
      budget:{ describeImage:$("cfg-img").value, describeGraph:$("cfg-graph").value, ocr:"ask", reflowPdf:"auto" } };
