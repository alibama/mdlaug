    ...
    return new Promise(function (resolve) {
      try {
        chrome.storage.local.get({ mdlaug_store: null, mdlaug_turso: { url: "", token: "", assessor: "" } }, function (s) {
          ...
          else { var lg = s.mdlaug_turso || {}, dflt = window.mDLAUG.store.defaultConfig(); cfg = (lg.url && lg.token) ? { storage: "turso-cloud", tursoUrl: lg.url, tursoToken: lg.token, assessor: lg.assessor || "" } : { storage: dflt.storage, tursoUrl: dflt.tursoUrl, tursoToken: dflt.tursoToken, assessor: lg.assessor || "" }; }
          resolve();
        });
      } catch (e) { resolve(); }
    });
