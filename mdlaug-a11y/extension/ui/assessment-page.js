    ...
    try{
      chrome.storage.local.get({ mdlaug_draft:null, mdlaug_store:null, mdlaug_turso:{url:"",token:"",assessor:""} }, function(s){
        if (s.mdlaug_store) { cfg = s.mdlaug_store; }
          ...
          var lg = s.mdlaug_turso || {}, dflt = window.mDLAUG.store.defaultConfig();
          if (lg.url && lg.token) cfg = { storage: "turso-cloud", tursoUrl: lg.url, tursoToken: lg.token, assessor: lg.assessor || "" };
          else cfg = { storage: dflt.storage, tursoUrl: dflt.tursoUrl, tursoToken: dflt.tursoToken, assessor: lg.assessor || "" };
        }
  ...
  function renderCards(card){
    ...
    card.forEach(function(s, idx){
      ...
      el.innerHTML =
        ...
        '<p class="sub"><span class="lvl '+s.level+'">Level '+s.level+'</span> '+
