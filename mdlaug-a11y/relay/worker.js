...
export default {
  async fetch(request, env) {
    ...
    // turso db show --url returns a libsql:// URL, but fetch() needs https://.
    var base = String(env.TURSO_URL).trim().replace(/^libsql:\/\//i, "https://").replace(/\/+$/, "");
    if (!/^https?:\/\//i.test(base)) base = "https://" + base;
    const upstream = base + "/v2/pipeline";
    let res;
    try {
