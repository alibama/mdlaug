...
turso db show mdlaug --url        # -> libsql://mdlaug-<org>.turso.io  (TURSO_URL)
...
3. Go to the Worker's **Settings → Variables and Secrets → Add**, and add these as
   ...
   - `TURSO_URL` — the URL from step 1. `turso db show` prints it as
     `libsql://…`; the Worker converts it to `https://` for you, but you can also
     just paste the `https://` form.
